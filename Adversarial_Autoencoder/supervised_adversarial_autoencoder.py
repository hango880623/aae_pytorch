...
#cfg = tf.ConfigProto() 
#cfg.gpu_options.per_process_gpu_memory_fraction = 0.5 # use50%GPU
...
def generate_image_grid(sess, op):
    ...
    """
    nx, ny = 16, 12
    random_inputs = np.random.randn(16, z_dim) * 5.
    sample_y = np.identity(12)
    plt.subplot()
    gs = gridspec.GridSpec(nx, ny, hspace=0.05, wspace=0.05)
    i = 0
    print(np.size(random_inputs))
    print(np.size(sample_y))
    for idx_r, r in enumerate(random_inputs):
        for idx_t, t in enumerate(sample_y):
            #print(idx_r,idx_t)
            ...
            ax = plt.subplot(gs[i])
            i += 1
            img = np.array(x.tolist()).reshape(256, 43)
            ...
            filename = str(idx_r)+'_'+str(idx_t)+'.wav'
            librosa.output.write_wav(save_audio_path+filename, y, sr)
    plt.show()
...
def train(train_model):
    ...
    if train_model:
        ...
        for d in data:
            ...
            tensor_x = torch.cat((tensor_x,element_x),0)
            list_y = list_y + [pitch_map(element_y)]

        tensor_x = tensor_x.reshape((len(data),input_dim))
        array_x = tensor_x.numpy()
        array_y = np.eye(n_labels)[list_y]
    ...
    input_images = tf.reshape(x_input, [-1, 256, 43, 1])
    generated_images = tf.reshape(decoder_output, [-1, 256, 43, 1])
    ...
    step = 0
    #dataset  = tf.data.Dataset.from_tensor_slices((tensor_x, tensor_y)).batch(batch_size).repeat(1).shuffle(20)
        ...
        if train_model:
            ...
            for i in range(n_epochs):
                ...
                for b in range(1, n_batches + 1):
                    ...
                    batch_x = array_x[(b-1)*batch_size:b * batch_size]
                    batch_y = array_y[(b-1)*batch_size:b * batch_size]
